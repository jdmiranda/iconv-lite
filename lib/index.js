...
iconv.encode = function encode (str, encoding, options) {
  ...
  // Fast path for UTF-8 without options
  if (!options && encoding && iconv._canonicalizeEncoding(encoding) === "utf8") {
    return Buffer.from(str, "utf8")
  }

  // Fast path for ASCII (check if all characters are < 128)
  if (!options && encoding && iconv._canonicalizeEncoding(encoding) === "ascii") {
    var isAscii = true
...
iconv.decode = function decode (buf, encoding, options) {
  ...
  // Fast path for UTF-8 without options (BOM handling is default for UTF-8)
  // Only use fast path when we're sure there's no BOM or when stripBOM option is explicitly false
  if (encoding && iconv._canonicalizeEncoding(encoding) === "utf8") {
    if (options && options.stripBOM === false) {
      return buf.toString("utf8")
    }
    // Check for UTF-8 BOM (0xEF, 0xBB, 0xBF) and strip it if present
    if (!options && buf.length >= 3 && buf[0] === 0xEF && buf[1] === 0xBB && buf[2] === 0xBF) {
